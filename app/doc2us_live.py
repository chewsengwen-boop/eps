...
class Doc2UsLiveRunner:
    ...
    def run_queue(self, queue: pd.DataFrame) -> LiveSubmitResult:
        ...
        with sync_playwright() as p:
            ...
            for candidate in ['/usr/bin/chromium-browser', '/snap/bin/chromium']:
    ...
    def _add_drug(self, page, row: pd.Series) -> None:
        ...
        med_input.fill(medication_name)
        page.wait_for_timeout(700)
        # Doc2Us requires selecting a search-result item, not only typing text.
        preferred = modal.locator('.search-result').filter(has_text=f'{medication_name} Tablet').first
        fallback = modal.locator('.search-result').filter(has_text=medication_name).first
        try:
            if preferred.count() and preferred.is_visible():
                preferred.click(force=True)
            elif fallback.count() and fallback.is_visible():
                fallback.click(force=True)
        except Exception:
            # Continue; form validation/confirmation will fail clearly if selection is mandatory.
            pass
